package com.flurry.android;

import android.content.*;
import android.os.*;
import android.text.*;

class da extends az
{
    private static final String d;
    
    static {
        d = da.class.getSimpleName();
    }
    
    @Override
    protected final Bundle a(final Context context) {
        Bundle p = bp.p(context);
        if (TextUtils.isEmpty((CharSequence)p.getString("com.flurry.admob.MY_AD_UNIT_ID"))) {
            db.d(da.d, "The meta-data should include not empty value for com.flurry.admob.MY_AD_UNIT_ID key in AndroidManifest.xml");
            p = null;
        }
        return p;
    }
    
    @Override
    protected final ao a(final Context context, final FlurryAds flurryAds, final cz cz, final AdUnit adUnit, final Bundle bundle) {
        if (context == null || flurryAds == null || cz == null || adUnit == null || bundle == null) {
            return null;
        }
        return new ak(context, flurryAds, cz, adUnit, bundle);
    }
    
    @Override
    protected final p a(final Context context, final FlurryAds flurryAds, final cz cz, final AdCreative adCreative, final Bundle bundle) {
        if (context == null || flurryAds == null || cz == null || adCreative == null || bundle == null) {
            return null;
        }
        return new ai(context, flurryAds, cz, adCreative, bundle);
    }
    
    @Override
    protected final av b() {
        return new av("GoogleAdMobAdsSdk", "6.2.1", "com.google.ads.InterstitialAd");
    }
    
    @Override
    protected final av c() {
        return new av("GoogleAdMobAdsSdk", "6.2.1", "com.google.ads.AdView");
    }
}
