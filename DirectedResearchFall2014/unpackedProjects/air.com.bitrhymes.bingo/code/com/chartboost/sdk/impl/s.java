...
public class s
{
    ...
    public s(final Context context, final int n) {
        super();
        this.j = new DataSetObserver() {
            public void onChanged() {
                for (int childCount = s.this.d.getChildCount(), i = 0; i < childCount; ++i) {
                    final List list = s.this.g.get(s.this.h.get(i));
                    final List<View> list2 = s.this.f.get(s.this.h.get(i));
                    final View child = s.this.d.getChildAt(i);
                    list.remove(child);
                    list2.add(child);
                }
                s.this.h.clear();
                s.this.d.removeAllViews();
                for (int count = s.this.e.getCount(), j = 0; j < count; ++j) {
                    final int itemViewType = s.this.e.getItemViewType(j);
                    final List<View> list3 = s.this.g.get(itemViewType);
                    final List<View> list4 = s.this.f.get(itemViewType);
                    s.this.h.add(itemViewType);
                    View view2;
                    if (!list4.isEmpty()) {
                    ...
                    final View view3 = s.this.e.getView(j, view2, (ViewGroup)s.this.d);
                    list3.add(view3);
                    ...
                    s.this.d.addView(view3, (ViewGroup$LayoutParams)linearLayout$LayoutParams);
                }
        ...
        this.d.setOnTouchListener((View$OnTouchListener)new View$OnTouchListener() {
            public boolean onTouch(final View view, final MotionEvent motionEvent) {
                try {
                    final View currentFocus = ((Activity)s.this.d.getContext()).getCurrentFocus();
