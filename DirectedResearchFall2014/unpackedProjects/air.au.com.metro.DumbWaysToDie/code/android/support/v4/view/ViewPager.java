...
public class ViewPager extends ViewGroup
{
        ...
        if (itemInfo2 != null) {
            ...
            if (position < itemInfo.position) {
                ...
                for (int i = position + 1; i <= itemInfo.position && n3 < this.mItems.size(); ++i) {
                    ItemInfo itemInfo3;
                    for (itemInfo3 = this.mItems.get(n3); i > itemInfo3.position && n3 < this.mItems.size() - 1; ++n3, itemInfo3 = this.mItems.get(n3)) {}
    ...
    public boolean canScrollHorizontally(final int n) {
        ...
        final int scrollX = this.getScrollX();
        if (n < 0) {
            return scrollX > (int)(clientWidth * this.mFirstOffset);
        }
        return n > 0 && scrollX < (int)(clientWidth * this.mLastOffset);
    }
                ...
                if (addNewItem != null) {
                    ...
                    for (int i = this.mCurItem - 1; i >= 0; --i) {
                        if (n3 >= n4 && i < max) {
                            ...
                            n3 += this.addNewItem(i, index2 + 1).widthFactor;
                            ++index;
                            if (index2 >= 0) {
                                itemInfo2 = this.mItems.get(index2);
                            }
                    ...
                    if (widthFactor < 2.0f) {
                        ...
                        if (clientWidth <= 0) {
                            ...
                            n5 = 2.0f + this.getPaddingRight() / clientWidth;
                        }
                        for (int j = 1 + this.mCurItem; j < count; ++j) {
                            if (widthFactor >= n5 && j > min) {
                                if (itemInfo3 == null) {
                                    break;
                                }
    ...
    void setCurrentItemInternal(int mCurItem, final boolean b, final boolean b2, final int n) {
        ...
        final boolean b3 = this.mCurItem != mCurItem;
        if (this.mFirstLayout) {
            this.mCurItem = mCurItem;
