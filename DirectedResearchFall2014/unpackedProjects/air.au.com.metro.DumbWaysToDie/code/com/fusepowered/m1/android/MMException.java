...
public final class MMException extends RuntimeException
{
    ...
    MMException(final int code) {
        super(getErrorCodeMessage(code));
        this.code = code;
    }
    
    MMException(final Exception cause) {
        super(cause);
        this.code = 2;
    }
    
    MMException(final String message, final int code) {
        super(message);
        this.code = code;
    }
    
    MMException(final String message, final Exception cause) {
        super(message, cause);
        this.code = 2;
    }
    
    static String getErrorCodeMessage(final int n) {
        switch (n) {
            default: {
                return "No error.";
            }
