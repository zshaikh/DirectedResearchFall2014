...
public class Facebook
{
    ...
    private void authorize(final Activity pendingAuthorizationActivity, final String[] a, final int requestCode, final SessionLoginBehavior loginBehavior, final Facebook$DialogListener facebook$DialogListener) {
        ...
        if (a != null) {
            pendingAuthorizationPermissions = a;
        }
        else {
            pendingAuthorizationPermissions = new String[0];
        }
        this.pendingAuthorizationPermissions = pendingAuthorizationPermissions;
        this.openSession(this.pendingOpeningSession, new Session$OpenRequest(pendingAuthorizationActivity).setCallback(new Facebook$1(this, facebook$DialogListener)).setLoginBehavior(loginBehavior).setRequestCode(requestCode).setPermissions(Arrays.asList(a)), this.pendingAuthorizationPermissions.length > 0);
    }
    ...
    private static String[] stringArray(final List list) {
    ...
    public final Session getSession() {
        while (true) {
            ...
            final Session$OpenRequest setPermissions = new Session$OpenRequest(this.pendingAuthorizationActivity).setPermissions((List)permissions);
            Label_0258: {
    ...
    String requestImpl(final String str, final Bundle bundle, final String s) {
        ...
        String s2;
        if (str != null) {
            s2 = Facebook.GRAPH_BASE_URL + str;
        }
        else {
