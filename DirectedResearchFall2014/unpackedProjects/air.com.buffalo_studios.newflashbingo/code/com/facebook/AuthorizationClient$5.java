    ...
    public void onBatchCompleted(final RequestBatch requestBatch) {
        try {
            ...
            if (this.val$fbids.size() == 2 && this.val$fbids.get(0) != null && this.val$fbids.get(1) != null && this.val$fbids.get(0).equals(this.val$fbids.get(1))) {
                authorizationClient$Result = AuthorizationClient$Result.createTokenResult(this.this$0.pendingRequest, AccessToken.createFromTokenWithRefreshedPermissions(this.val$pendingResult.token, this.val$tokenPermissions));
            }
            else {
