...
public class e implements ComponentCallbacks, View$OnCreateContextMenuListener
{
    ...
    private static final at<String, Class<?>> sClassMap;
    j mActivity;
    boolean mAdded;
    View mAnimatingAway;
    Bundle mArguments;
    ...
    public static e instantiate(final Context context, final String s, final Bundle mArguments) {
        try {
            ...
            final e e = (e)loadClass.newInstance();
            if (mArguments != null) {
    ...
    static boolean isSupportFragmentClass(final Context context, final String name) {
        try {
            Class<?> loadClass = e.sClassMap.get(name);
            if (loadClass == null) {
                loadClass = context.getClassLoader().loadClass(name);
                e.sClassMap.put(name, loadClass);
            }
